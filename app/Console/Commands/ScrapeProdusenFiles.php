...
class ScrapeProdusenFiles extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'produsen:scrape-files
                            {--id= : Specific Produsen Form ID to scrape}
                            {--all : Scrape all Produsen forms}';
     ...
     * @var string
     */
    protected $description = 'Scrape and download files from Produsen DG form meta data';
    ...
    {
        $this->info('🚀 Starting Produsen DG Files Scraping...');

        $query = ProdusenForm::query();
        ...
        if ($forms->isEmpty()) {
            $this->warn('No Produsen forms found.');
            return 0;
        }

        $this->info("Found {$forms->count()} Produsen form(s) to process.");
        ...
        foreach ($forms as $form) {
            $this->newLine();
            $this->info("📋 Processing: {$form->nama_instansi} (ID: {$form->id})");

            $result = $this->processProdusenForm($form);
    ...
    /**
     * Process a single Produsen form
     */
    protected function processProdusenForm(ProdusenForm $form): array
    {
        $downloaded = 0;
        $failed = 0;

        // Create folder name from Produsen name
        $folderName = $this->sanitizeFolderName($form->nama_instansi);
        $basePath = "scrapping_script/produsen_form/{$folderName}";
    ...
    protected function sanitizeFolderName(string $name): string
    {
        ...
        $name = preg_replace('/^(direktorat|dir\.?|balai|bpkh)\s*/i', '', $name);
        ...
        // Prefix with produsen_
        return 'produsen_' . $name;
    }
